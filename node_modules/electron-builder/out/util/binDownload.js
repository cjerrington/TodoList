...
function downloadFpm(version, osAndArch) {
    return getBin("fpm", `fpm-${ version }-${ osAndArch }`, `https://github.com/develar/fpm-self-contained/releases/download/v${ version }/${ `fpm-${ version }-${ osAndArch }` }.7z`).then(it => path.join(it, "fpm"));
}
...
function doGetBin(name, dirName, url, sha2) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield bluebird_1.Promise.all([fs_extra_p_1.rename(isOldMethod ? path.join(tempUnpackDir, dirName) : tempUnpackDir, dirPath).catch(e => {
        ...
        if (isOldMethod) {
            yield fs_extra_p_1.remove(tempUnpackDir);
        }

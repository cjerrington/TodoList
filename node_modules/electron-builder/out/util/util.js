...
function installDependencies(appDir, electronVersion) {
    ...
    return log_1.task(`${ command === "install" ? "Installing" : "Rebuilding" } app dependencies for arch ${ arch } to ${ appDir }`, spawnNpmProduction(command, appDir, Object.assign({}, process.env, {
...
function exec(file, args, options) {
    if (exports.debug.enabled) {
        exports.debug(removePassword(`Executing ${ file } ${ args == null ? "" : args.join(" ") }`));
    }
    return new bluebird_1.Promise((resolve, reject) => {
        child_process_1.execFile(file, args, options, function (error, stdout, stderr) {
            if (error == null) {
                ...
                let message = chalk_1.red(removePassword(error.message));
                if (stdout.length !== 0) {
...
function doSpawn(command, args, options, pipeInput) {
    ...
    if (options.stdio == null) {
        options.stdio = [pipeInput ? "pipe" : "ignore", exports.debug.enabled ? "inherit" : "pipe", "inherit"];
    }
    if (exports.debug.enabled) {
...
function spawn(command, args, options, pipeInput) {
    return new bluebird_1.Promise((resolve, reject) => {
...
function getElectronVersion(packageData, packageJsonPath) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (e.code !== "ENOENT") {
                log_1.warn(`Cannot read electron version from electron-prebuilt package.json: ${ e.message }`);
            }
        }

...
function downloadCertificate(urlOrBase64) {
    const tempFile = path.join(os_1.tmpdir(), `${ util_1.getTempName() }.p12`);
    if (urlOrBase64.startsWith("https://")) {
        return httpRequest_1.download(urlOrBase64, tempFile).thenReturn(tempFile);
    } else if (urlOrBase64.startsWith("file://")) {
        return bluebird_1.Promise.resolve(urlOrBase64.substring("file://".length));
    } else {
        return fs_extra_p_1.outputFile(tempFile, new Buffer(urlOrBase64, "base64")).thenReturn(tempFile);
    }
}
...
function createKeychain(keychainName, cscLink, cscKeyPassword, cscILink, cscIKeyPassword) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return yield promise_1.executeFinally(bluebird_1.Promise.all([bluebird_1.Promise.map(certLinks, (link, i) => downloadCertificate(link).then(it => certPaths[i] = it)), bluebird_1.Promise.mapSeries([["create-keychain", "-p", keychainPassword, keychainName], ["unlock-keychain", "-p", keychainPassword, keychainName], ["set-keychain-settings", "-t", "3600", "-u", keychainName]], it => util_1.exec("security", it))]).then(() => importCerts(keychainName, certPaths, [cscKeyPassword, cscIKeyPassword].filter(it => it != null))), errorOccurred => {
            const tasks = certPaths.map((it, index) => certLinks[index].startsWith("https://") ? fs_extra_p_1.deleteFile(it, true) : bluebird_1.Promise.resolve());
            if (errorOccurred) {
